...
# -------------------------------------------------------------
PERSON_CLASS_ID = 0
PERSON_THRESH = 0.50  # เงื่อนไขตามโจทย์: >= 50% ให้ไฟแดงติด
...
def parse_args():
    ...
    p.add_argument('--thresh', type=float, default=0.5, help='confidence threshold (for drawing/filtering)')
...
class TrafficLight:
    def __init__(self, pin_r, pin_y, pin_g, invert=False):
        ...
        try:
            ...
            # Define ON/OFF levels based on invert
...
def main():
    ...
    min_thresh = float(args.thresh)  # ใช้สำหรับการวาด/กรองผลทั่วไป
    ...
    labels = model.names  # dict {id:name} หรือ list
    ...
    # Traffic light controller (optional)
    tl = None
    if args.use_gpio:
